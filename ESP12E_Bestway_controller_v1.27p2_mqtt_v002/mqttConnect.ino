void MQTT_Connect()
{
  ...
  */
  if (MQTTclient.connect(mqtt_client_id.c_str(), mqtt_username, mqtt_password, (base_mqtt_topic + "/Status").c_str(), 0, 1, "Dead"))
  {
    ...
    MQTTclient.publish((base_mqtt_topic + "/Client_ID").c_str(), String(mqtt_client_id).c_str(), true);              //Client ID
    MQTTclient.loop();


    // ... and then re/subscribe to the watched topics
    MQTTclient.subscribe((base_mqtt_topic + "/command").c_str());   // Watch the .../command topic for incoming MQTT messages
    MQTTclient.loop();
